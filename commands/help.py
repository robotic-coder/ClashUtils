...
async def help(ctx: discord.ext.commands.Context, *args):
	...
	if len(args) == 0:
		...
		embed.add_field(name="Usage", value=ctx.prefix+command.name+" "+command.usage, inline=False)
		embed.add_field(name="Example", value=ctx.prefix+command.name+" "+command.example, inline=False)
